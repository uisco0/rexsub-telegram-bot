...
def check_data_integrity():
    """فحص وإصلاح سلامة بيانات المستخدمين"""
    try:
        ...
        for user_id, user_data in data.items():
            ...
            if 'points' not in user_data:
                user_data['points'] = 0
                needs_fix = True
                logger.info(f"⚠️ إصلاح نقاط المستخدم {user_id}: إضافة حقل النقاط")
            ...
            if not isinstance(user_data['points'], (int, float)):
                try:
                    ...
                    needs_fix = True
                    logger.info(f"⚠️ إصلاح نقاط المستخدم {user_id}: تحويل إلى عدد صحيح")
                except:
                    user_data['points'] = 0
                    needs_fix = True
                    logger.info(f"⚠️ إصلاح نقاط المستخدم {user_id}: تعيين إلى 0")
            
            # التأكد من الحقول الأخرى
            default_fields = {
                'lang': 'ar',
                'referred_by': None,
                'rewarded': False,
        ...
        if needs_fix:
            logger.info(f"✅ إصلاح بيانات {len(data)} مستخدم")
...
def load_json(filename, default):
    """تحميل بيانات JSON مع معالجة الأخطاء"""
    try:
        if os.path.exists(filename):
            with open(filename, "r", encoding="utf-8") as f:
                data = json.load(f)
                # فحص ما إذا كان الملف فارغاً أو تالفاً
                if data is None:
                    logger.info(f"⚠️ ملف {filename} فارغ، إرجاع القيمة الافتراضية")
                    return default
                return data
        return default
    except json.JSONDecodeError as e:
        logger.error(f"❌ خطأ في فك تشفير JSON في ملف {filename}: {e}")
...
def save_json(filename, data):
    """حفظ بيانات JSON مع معالجة الأخطاء"""
    try:
...
def is_admin(user_id):
    try:
        ...
        logger.error(f"❌ خطأ في التحقق من صلاحيات المدير {user_id}: {e}")
        return False
...
def ensure_user_data(user_id):
    """التأكد من وجود بيانات المستخدم وإنشاءها إذا لم تكن موجودة"""
    ...
    if user_id_str not in data:
        data[user_id_str] = {
            'points': 0, 
            'lang': 'ar',  # تعيين اللغة الافتراضية
            'referred_by': None, 
            'rewarded': False, 
            'purchases': 0, 
            'spent_points': 0,
            'lang_prompt_sent': True,  # تم إرسال الرسالة
            'unknown_command_sent': False,
            'created_at': time.time(),
            'last_seen': time.time()
        }
        save_json(DB_FILE, data)
        logger.info(f"✅ تم إنشاء مستخدم جديد: {user_id_str}")
    else:
        ...
        needs_update = False
        for field, default_value in default_fields.items():
            if field not in data[user_id_str]:
                data[user_id_str][field] = default_value
                needs_update = True
        
        # تحديث وقت آخر ظهور
        data[user_id_str]['last_seen'] = time.time()
...
def add_netflix_accounts(accounts_list, max_users):
    ...
    logger.info(f"✅ تم إضافة {len(accounts_list)} حساب نتفلكس جديد")
...
def delete_netflix_account(index):
    ...
    if 0 <= index < len(accounts):
        del accounts[index]
        save_json(NETFLIX_FILE, accounts)
        logger.info(f"✅ تم حذف حساب نتفلكس رقم {index}")
        return True
    return False
...
def add_icloud_account(account, photo_id, text, max_users):
    ...
    logger.info(f"✅ تم إضافة حساب iCloud جديد: {account.split(':')[0]}")
...
def delete_icloud_account(index):
    accounts = load_json(ICLOUD_FILE, [])
    if 0 <= index < len(accounts):
        account_email = accounts[index]['account'].split(':')[0]
        del accounts[index]
        save_json(ICLOUD_FILE, accounts)
        logger.info(f"✅ تم حذف حساب iCloud: {account_email}")
        return True
    return False
...
def show_members_list(admin_id):
    ...
    stats_text = STRINGS['ar']['members_stats'].format(
        total=total_members,
...
def search_member(admin_id):
    """البحث عن عضو معين"""
    msg = bot.send_message(admin_id, STRINGS['ar']['search_prompt'], parse_mode="Markdown")
...
def process_member_search(message):
    ...
    for user_id, user_data in data.items():
        try:
            ...
            try:
                user_info = bot.get_chat(int(user_id))
                if user_info.username and f"@{user_info.username}".lower() == search_query.lower():
                    found_members.append((user_id, user_data))
                    continue
    ...
    if not found_members:
        bot.send_message(admin_id, STRINGS['ar']['member_not_found'], parse_mode="Markdown")
        return
    ...
    for user_id, user_data in found_members[:5]:  # عرض أول 5 فقط
        try:
            ...
            member_details = f"{STRINGS['ar']['member_found']}"
            member_details += f"👤 **الاسم:** {full_name}\n"
            member_details += f"📱 **اليوزر:** {username}\n"
            member_details += f"💰 **النقاط:** {points}\n"
            member_details += f"🛍️ **المشتريات:** {purchases}\n"
            member_details += f"💸 **النقاط المستخدمة:** {spent_points}\n"
            member_details += f"👥 **الإحالات:** {referrals}\n"
            member_details += f"🌐 **اللغة:** {'عربي' if lang == 'ar' else 'إنجليزي'}\n"
            member_details += f"🆔 **المعرف:** `{user_id}`\n"
            member_details += f"📅 **تاريخ الإنشاء:** {datetime.fromtimestamp(user_data.get('created_at', time.time())).strftime('%Y-%m-%d %H:%M:%S')}\n"
            member_details += f"━━━━━━━━━━━━━━"
            
            bot.send_message(admin_id, member_details, parse_mode="Markdown")
            time.sleep(0.5)
...
def handle_msg(message):
    ...
    if is_menu_command:
        if user_text == s['my_account']:
            ...
            data = check_data_integrity()
            user_data = data.get(user_id, user_data)
...
def callback_handler(call):
    ...
    # إعادة تعيين البيانات إذا كانت اللغة غير موجودة
    if not lang:
        lang = 'ar'
        user_data['lang'] = 'ar'
    ...
    if call.data.startswith('lang_'):
        ...
        if is_subscribed(user_id):
            ...
            bot.answer_callback_query(call.id, "❌ الرجاء الاشتراك في جميع القنوات أولاً!", show_alert=True)
            ...
            if call.data == 'buy_netflix':
                ...
                if account:
                    try:
                        ...
                        bot.send_message(user_id, "❌ خطأ في تنسيق الحساب. الرجاء التواصل مع الدعم.")
                        return
                    
                    # حفظ النقاط قبل التعديل
                    old_points = user_data.get('points', 0)
                    user_data['purchases'] += 1
                    user_data['spent_points'] += required_points
                    user_data['points'] = max(0, old_points - required_points)
        ...
        if selected_country:
            ...
            if pts >= PRODUCT_PRICES['buy_telegram']:
                ...
                old_points = user_data.get('points', 0)
                user_data['points'] = max(0, old_points - PRODUCT_PRICES['buy_telegram'])
...
🔒 Code is non-repeatable and unique to you"""
        ...
        try:
            ...
            logger.error(f"❌ خطأ في تعديل الرسالة: {e}")
            bot.send_message(user_id, store_text, reply_markup=markup, parse_mode="Markdown")
        ...
        if 0 <= index < len(accounts) and pts >= PRODUCT_PRICES['buy_icloud']:
            ...
            if acc['remaining_users'] > 0 and str(user_id) not in acc['users_received']:
                ...
                user_data['points'] = max(0, old_points - PRODUCT_PRICES['buy_icloud'])
                ...
                try:
                    ...
                    logger.error(f"❌ خطأ في تعديل صورة iCloud: {e}")
            else:
    ...
    # ============ وظائف الإدارة ============
        ...
        if is_admin(user_id):
            purchases = get_purchase_history(100)
        ...
        if is_admin(user_id):
            ...
            if admin_to_remove in admins:
                ...
                logger.info(f"✅ تم إزالة المدير: {admin_to_remove}")
            else:
        ...
        if is_admin(user_id):
            try:
                ...
                logger.error(f"❌ خطأ في حذف حساب نتفلكس: {e}")
                bot.send_message(user_id, "❌ حدث خطأ أثناء حذف الحساب.")
        ...
        if is_admin(user_id):
            try:
                ...
                logger.error(f"❌ خطأ في حذف حساب iCloud: {e}")
                bot.send_message(user_id, "❌ حدث خطأ أثناء حذف الحساب.")
...
def process_transfer_amount(message, target_user):
    ...
    if user_found not in data:
        data[user_found] = {'points': 0, 'lang': 'ar', 'referred_by': None, 'rewarded': False, 'purchases': 0, 'spent_points': 0, 'lang_prompt_sent': False, 'unknown_command_sent': False, 'created_at': time.time(), 'last_seen': time.time()}
    
    data[user_found]['points'] += amount
    save_json(DB_FILE, data)
    
    try:
        bot.send_message(int(user_found), f"🎁 **مبروك! استلمت نقاط جديدة**\n━━━━━━━━━━━━━━\nتمت إضافة **{amount}** نقطة إلى حسابك من قبل الإدارة.\n🚀 يمكنك استخدامها الآن في المتجر.", parse_mode="Markdown")
    except Exception as e:
        logger.error(f"❌ خطأ في إرسال إشعار تحويل النقاط: {e}")
...
# فحص سلامة البيانات عند التشغيل
